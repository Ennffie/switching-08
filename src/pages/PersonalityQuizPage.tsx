...
const quizData: QuizOption[] = [
  {
    emoji: '🧘',
    text: '匿喺角落',
    description: '享受一個人嘅寧靜'
  },
  {
    emoji: '👥',
    text: '同幾個朋友',
    description: '小圈子傾偈最舒服'
  },
  {
    emoji: '🎉',
    text: '融入派對',
    description: '隨遇而安，開心就好'
  },
  {
    emoji: '🎤',
    text: '做全場焦點',
    description: '帶動氣氛，成為主角'
  }
];
...
const PersonalityQuizPage = () => {
  ...
  useEffect(() => {
    const index = Math.min(
      Math.floor((sliderValue / 100) * quizData.length),
      quizData.length - 1
    );
      ...
      <div className="quiz-content">
        ...
        <div className="selected-option-container">
          <div className="selected-option-card">
            <div className="selected-emoji">{quizData[selectedIndex].emoji}</div>
            <div className="selected-text">{quizData[selectedIndex].text}</div>
            <div className="selected-description">{quizData[selectedIndex].description}</div>
          </div>
        </div>
        ...
        <div className="options-preview">
          {quizData.map((_option, index) => (
            <div 
              ...
              onClick={() => setSliderValue((index / (quizData.length - 1)) * 100)}
            >
              <span className="dot-emoji">{quizData[index].emoji}</span>
            </div>
          ))}
        </div>
      </div>
